...
if(!(GPIOE->IDR&(1<<6))) //если замкнуто 1-е реле
	{
  if(isp_in_cnt[0]<100)isp_in_cnt[0]++;
	if(isp_in_cnt[0]>=100)isp_in_cnt[0]=100;
	}
else 
	{
  if(isp_in_cnt[0])isp_in_cnt[0]--;
	if(isp_in_cnt[0]<0)isp_in_cnt[0]=0;
	}

if(!(GPIOE->IDR&(1<<7))) //если замкнуто 2-е реле
	{
  if(isp_in_cnt[1]<100)isp_in_cnt[1]++;
	if(isp_in_cnt[1]>=100)isp_in_cnt[1]=100;
	}
else 
	{
  if(isp_in_cnt[1])isp_in_cnt[1]--;
	if(isp_in_cnt[1]<0)isp_in_cnt[1]=0;
	}
	
if(!(GPIOD->IDR&(1<<0))) //если замкнуто 3-е реле
	{
  if(isp_in_cnt[2]<100)isp_in_cnt[2]++;
	if(isp_in_cnt[2]>=100)isp_in_cnt[2]=100;
	}
else 
	{
